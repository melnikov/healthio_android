...
public class TreatmentAddForm extends Activity implements SeekBar.OnSeekBarChangeListener {
	TreatmentsLauncher parentActivity;
	Intent intent;
	Bundle bundle;
	int identIntent=0;
	String titleIntent="";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.treatment_add_form);
        intent=getIntent();
	...
	public void submit(View target) throws ParseException {
		...
		int lengthItems=twotabObject.size();
		int maxID=0;
		for (int i=0; i<lengthItems; i++) {
			Object item=(Object) twotabObject.get(i);
		...
		int medicalSystem=treatmentSystemNumber;
		...
		if (title=="" || subtitle=="" || startDate==null || endDate==null || treatmentStrength=="" || repeatIntent==-1 || treatmentType==0 || medicalSystem==0 || endDate==null || numStartdate>numEnddate) {
			Toast.makeText(getApplicationContext(), "You do not fill in all fields!", Toast.LENGTH_SHORT).show();
		}
		else {
			DateComponentIntentTreatment repeatThis=new DateComponentIntentTreatment();
			...
			twotabObject.add(new TreatmentsItem(title, subtitle, startDate, endDate, idSection, repeat, repeatTimes, severity, lastUpdateDate, ID, treatmentType, treatmentStrength, medicalSystem));
			// SQLITE-query (begin)
			...
			cv.put("system", medicalSystem);
			db.insertOrThrow("treatments", null, cv);
